...
async function removeActionItem(id){
  ...
  actionItems.RecordData = actionItems.RecordData.filter(e => e.replaceAll(' ', '_') != id)
  await update("0","ActionItems",actionItems.RecordData);
}
...
async function fetchData(){
  ...
  if(actionItems){
    for (var i in actionItems.RecordData){
      ...
      fc.innerHTML += '<input class="form-check-input" type="checkbox" value="" id=' + actionItems.RecordData[i].replaceAll(' ', '_') +' onclick="removeActionItem(id)">';
...
async function signup(form){
  ...
  if(item){
    ...
    myHeaders.append("Content-Type", "application/json");
    // using built in JSON utility package turn object to string and store in a variable
    raw = JSON.stringify({"age":form.age.value,"name":form.name.value,"username":form.username.value,"password":form.password.value});
    // create a JSON object with parameters for API call and store in a variable
    requestOptions = {
        method: 'POST',
        headers: myHeaders,
